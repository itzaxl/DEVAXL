...
local function playClick()   _snd("6042053626", 0.28) end
local function playToggle()  _snd("9119713951", 0.22) end
local function playOpen()    _snd("6895079853", 0.18) end
...
local CONFIG = {
    Colors = {
        ...
        Stroke = Color3.fromRGB(255, 185, 30),
        ...
        SwitchOn = Color3.fromRGB(255, 185, 30),
        SectionHeader = Color3.fromRGB(18, 18, 18),
        ESPHighlight = Color3.fromRGB(255, 185, 30),
        PlotESPHighlight = Color3.fromRGB(0, 255, 120),
        BrainrotESPHighlight = Color3.fromRGB(255, 185, 30),
    },
    UI = {
...
title.Text = "⭐ AXL HUB BRAINROT NEW 5V [FREE]"
...
local function setupJumpPowerControl(parent)
    ...
    local jumpSwitch = createSwitch(parent, "تجاوز القفز", _G.SavedToggleStates and _G.SavedToggleStates.Jump or false, function(on)
        local success, _ = pcall(function()
...
local function setupSpeedControl(parent)
    ...
    local speedSwitch = createSwitch(parent, "تسريع اللاعب", _G.SavedToggleStates and _G.SavedToggleStates.Speed or false, function(on)
        if on then
            enableSpeed()
            -- Auto-create side toggle when enabled
...
local function setupUnhittableControl(parent)
    ...
    unhittableSwitch = createSwitch(parent, "تجاوز الارتفاع", false, function(on)
        local success, _ = pcall(function()
...
local function setupResizeControl(parent)
    ...
    resizeSwitch = createSwitch(parent, "طويل", false, function(on)
        local success, _ = pcall(function()
            isResizeActive = on
            if humanoidRootPart then
...
local function setupFlingControl(parent)
    ...
    flingSwitch = createSwitch(parent, "الرمي (ملغي)", false, function(on)
        if on then
            enableFling()
        else
            disableFling()
...
createTabButton("🏠 الرئيسية", "Home", "rbxassetid://6031265976")
createTabButton("🚀 الحركة", "Movement", "rbxassetid://6035047409")
createTabButton("👁 المرئيات", "Visual", "rbxassetid://6031280882")
createTabButton("🔧 ملغي", "Patched", "rbxassetid://6031289451")
createTabButton("🌐 السيرفر", "Server", "rbxassetid://6031068421")
createTabButton("⚡ ديسينك", "Desync", "rbxassetid://6031094677")
...
welcomeLabel.Text = "⭐ AXL HUB BRAINROT NEW 5V [FREE]\n\nأهلاً! أفضل سكريبت مجاني لـ Steal a Brainrot\nيتحدث كل أسبوع | انضم لمجتمعنا: " .. CONFIG.DiscordLink
...
local function createButton(parent, text, callback)
    ...
    button.MouseButton1Click:Connect(function() playClick() if callback then pcall(callback) end end)
    
    -- Mobile support - Touch events
    button.TouchTap:Connect(function() if callback then pcall(callback) end end)
...
local floatSwitch = createSwitch(_G.movementSection, "طيران بطيء", CONFIG.Movement.Float.Enabled, function(on)
...
local platformSwitch = createSwitch(_G.movementSection, "منصة صاعدة", CONFIG.Movement.Rise.Enabled, function(on)
...
local helicopterSwitch = createSwitch(_G.movementSection, "هيليكوبتر", CONFIG.Movement.Helicopter.Enabled, function(on)
...
local function createFlingButton()
    flingToggleButton = createButton(_G.movementSection, "ارمه! 💥", function()
        local success, err = pcall(function()
...
local serverHopSwitch = createSwitch(_G.serverSection, "تنقل تلقائي", false, function(on)
...
-- Server Buttons
createButton(_G.serverSection, "إعادة الانضمام", _G.rejoinServer)
createButton(_G.serverSection, "السيرفر الأكبر", _G.joinBiggestServer)
createButton(_G.serverSection, "السيرفر الأصغر", _G.joinSmallestServer)
...
-- Settings Section
createSectionHeader(settingsContent, "إعدادات الـ ESP")
...
-- Plot ESP Toggles
createSectionHeader(settingsContent, "إعدادات Plot ESP")
...
local ragdollDesyncSwitch = createSwitch(_G.movementSection, "ديسينك الجسم", false, function(on)
...
createSectionHeader(settingsContent, "إعادة الضبط")
...
local originalLaserCapeSwitch = createSwitch(_G.movementSection, "كيب الليزر التلقائي", false, function(on)
    if on then
...
--=========================================================
local AXL_GOLD      = Color3.fromRGB(255, 190,  35)
local AXL_GOLD_D    = Color3.fromRGB(150, 110,  15)
local AXL_BG        = Color3.fromRGB(10,  10,  10)
local AXL_ROW       = Color3.fromRGB(18,  18,  18)
local AXL_GREEN     = Color3.fromRGB(45,  200,  85)
local AXL_RED_C     = Color3.fromRGB(215,  50,  50)
local AXL_WHITE     = Color3.fromRGB(255, 255, 255)
local AXL_SUB       = Color3.fromRGB(130, 130, 130)
...
local function axlCreatePart(pos, size, parent)
    local p = Instance.new("Part", parent)
    p.Size = size; p.Position = pos + Vector3.new(0,10,0)
    p.Anchored = true; p.Material = Enum.Material.Neon
    p.Color = AXL_GOLD_D; p.Transparency = 0.2
    local sb = Instance.new("SelectionBox", p)
    sb.Adornee = p; sb.Color3 = AXL_GOLD; sb.LineThickness = 0.05
    TweenService:Create(p, TweenInfo.new(0.3), {Position = pos}):Play()
    return p
end
local function axlStartBuild()
    axlClearBuild()
    local model = Instance.new("Model", workspace); model.Name = AXL_MAP_NAME
    local data = {
...
-- TRANSPARENCY
local AXL_TRANS_PATS = {"Side","Structure","Base","Home"}
local function axlIsTarget(n)
    for _, p in ipairs(AXL_TRANS_PATS) do if string.find(n,p) then return true end end
    return false
end
local function axlApplyTransparency()
    local ok, plots = pcall(function() return workspace:WaitForChild("Plots",5) end)
    if not ok or not plots then return end
    for _, plot in ipairs(plots:GetChildren()) do
        if plot:IsA("Model") then
            for _, d in ipairs(plot:GetDescendants()) do
                if d:IsA("BasePart") and axlIsTarget(d.Name) then d.Transparency = 0.9 end
            end
        end
    end
end
if AXL_ENABLED.TRANSPARENCY then task.spawn(axlApplyTransparency) end
...
local axlESPDet = {}; local axlESPObjs = {}
local function axlRemESP(m)
    if axlESPObjs[m] then for _,o in ipairs(axlESPObjs[m]) do pcall(function() o:Destroy() end) end; axlESPObjs[m]=nil end
    axlESPDet[m]=nil
end
local function axlCreateESP(model)
    if axlESPDet[model] then return end; axlESPDet[model]=true; axlESPObjs[model]={}
    local hrp = model:FindFirstChild("HumanoidRootPart") or model:FindFirstChildWhichIsA("BasePart")
    if not hrp then return end
    local hl = Instance.new("Highlight", model)
    hl.FillColor=AXL_GOLD; hl.OutlineColor=AXL_WHITE; hl.FillTransparency=0.5
    table.insert(axlESPObjs[model], hl)
    local bill = Instance.new("BillboardGui", hrp)
    bill.Size=UDim2.new(0,200,0,40); bill.StudsOffset=Vector3.new(0,6,0); bill.AlwaysOnTop=true
    table.insert(axlESPObjs[model], bill)
    local frame = Instance.new("Frame", bill)
    frame.Size=UDim2.new(1,0,1,0); frame.BackgroundColor3=AXL_BG; frame.BackgroundTransparency=0.2
    Instance.new("UICorner",frame).CornerRadius=UDim.new(0,8)
    local st = Instance.new("UIStroke",frame); st.Color=AXL_GOLD; st.Thickness=1.5
    local lbl = Instance.new("TextLabel",frame)
    lbl.Size=UDim2.new(1,0,1,0); lbl.BackgroundTransparency=1
    lbl.Text="💎 نادر: "..model.Name; lbl.TextColor3=AXL_WHITE
...
local function axlClearAllESP()
    for m,_ in pairs(axlESPDet) do axlRemESP(m) end
    axlESPDet={}; axlESPObjs={}
end
local axlScanT = 0
RunService.Heartbeat:Connect(function(dt)
    axlScanT = axlScanT + dt
    if axlScanT < 2 then return end; axlScanT = 0
    if not AXL_ENABLED.ESP_RARE then return end
    for _, v in ipairs(workspace:GetDescendants()) do
        if v:IsA("Model") then
            for _, nm in ipairs(AXL_RARE) do
                if v.Name == nm then axlCreateESP(v); break end
            end
        end
    end
end)
...
-- ANTI-AFK
local axlLastAct = tick()
UserInputService.InputBegan:Connect(function() axlLastAct = tick() end)
task.spawn(function()
    while true do
        task.wait(2)
        if AXL_ENABLED.ANTI_AFK and tick()-axlLastAct > 10 then
            local c=player.Character
            local h=c and c:FindFirstChildOfClass("Humanoid")
            if h then h.Jump=true; h:Move(Vector3.new(math.random(-1,1),0,math.random(-1,1)),false) end
            axlLastAct=tick()
        end
    end
end)
...
local function axlDoMove()
    if axlIsMoving then return end; axlIsMoving=true
    local c=player.Character
    if not c then axlIsMoving=false; return end
...
task.spawn(function()
    while true do task.wait(5)
        if AXL_ENABLED.RANDOM_MOVE then task.spawn(axlDoMove) end
    end
end)
...
local function axlSwitch(label, sub, key, onEn, onDis)
    local row = Instance.new("Frame", _G.axlSection)
    row.Size = UDim2.new(1,0,0,52); row.BackgroundColor3=AXL_ROW; row.BorderSizePixel=0
    Instance.new("UICorner",row).CornerRadius=UDim.new(0,8)
    local rs=Instance.new("UIStroke",row); rs.Color=AXL_GOLD_D; rs.Thickness=1; rs.Transparency=0.5
    local lbl=Instance.new("TextLabel",row); lbl.BackgroundTransparency=1
    lbl.Position=UDim2.new(0,10,0,6); lbl.Size=UDim2.new(0.75,0,0,20)
    lbl.Text=label; lbl.Font=Enum.Font.GothamBold; lbl.TextSize=12
    lbl.TextColor3=AXL_WHITE; lbl.TextXAlignment=Enum.TextXAlignment.Left
    local sublbl=Instance.new("TextLabel",row); sublbl.BackgroundTransparency=1
    sublbl.Position=UDim2.new(0,10,0,28); sublbl.Size=UDim2.new(0.75,0,0,16)
    sublbl.Text=sub; sublbl.Font=Enum.Font.Gotham; sublbl.TextSize=10
    sublbl.TextColor3=AXL_SUB; sublbl.TextXAlignment=Enum.TextXAlignment.Left
    local swBG=Instance.new("Frame",row); swBG.Size=UDim2.new(0,42,0,22)
    swBG.Position=UDim2.new(1,-52,0.5,-11); swBG.BorderSizePixel=0
    swBG.BackgroundColor3=AXL_ENABLED[key] and AXL_GOLD or Color3.fromRGB(50,50,50)
    Instance.new("UICorner",swBG).CornerRadius=UDim.new(1,0)
    local knob=Instance.new("Frame",swBG); knob.Size=UDim2.new(0,16,0,16); knob.BorderSizePixel=0
    knob.Position=AXL_ENABLED[key] and UDim2.new(1,-19,0.5,-8) or UDim2.new(0,3,0.5,-8)
    knob.BackgroundColor3=AXL_WHITE; Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    local hit=Instance.new("TextButton",row); hit.Size=UDim2.new(1,0,1,0)
    hit.BackgroundTransparency=1; hit.Text=""
    local function tog()
        AXL_ENABLED[key]=not AXL_ENABLED[key]; local on=AXL_ENABLED[key]
        playToggle()
        TweenService:Create(swBG,TweenInfo.new(0.15),{BackgroundColor3=on and AXL_GOLD or Color3.fromRGB(50,50,50)}):Play()
    ...
    hit.MouseButton1Click:Connect(tog); hit.TouchTap:Connect(tog)
end

createSectionHeader(_G.axlSection, "🔧 ميزات AXL")
axlSwitch("🏗️ نظام البناء", "بناء سينمائي 360°", "BUILD",
    function() axlStartBuild() end, function() axlClearBuild() end)
axlSwitch("🌫️ شفافية المزارع", "إخفاء جدران Side/Base", "TRANSPARENCY",
    function() task.spawn(axlApplyTransparency) end, nil)
axlSwitch("💎 ESP النوادر", "رصد 32 براينروت نادر", "ESP_RARE",
    nil, function() axlClearAllESP() end)
axlSwitch("🛡️ Anti-AFK", "حماية من الطرد التلقائي", "ANTI_AFK", nil, nil)
axlSwitch("🤖 حركة عشوائية", "تجول ذكي بـ PathFinding", "RANDOM_MOVE", nil, nil)

createSectionHeader(_G.axlSection, "🚀 أوامر سريعة")
createButton(_G.axlSection, "🚀 ابنِ الخريطة الآن", function() axlStartBuild() end)
createButton(_G.axlSection, "🗑️ احذف الخريطة", function() axlClearBuild() end)
createButton(_G.axlSection, "👁️ أعد رصد النوادر", function() axlClearAllESP() end)
createButton(_G.axlSection, "🌫️ طبّق الشفافية الآن", function() task.spawn(axlApplyTransparency) end)
